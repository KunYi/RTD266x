...
static const struct FlashDesc FlashDevices[] = {
...
uint32_t SPICommonCommand(enum ECommondCommandType cmd_type,
    uint8_t cmd_code,
    uint8_t num_reads,
    uint8_t num_writes,
...
static const struct FlashDesc* FindChip(uint32_t jedec_id) {
  const struct FlashDesc* chip = FlashDevices;
...
static uint8_t* ReadFile(const char *file_name, uint32_t* size) {
  ...
  if (file_size > 8*1024*1024) {
    printf("This file looks to big %ld\n", file_size);
    fclose(file);
    return result;
  }

  result = (uint8_t*) malloc(file_size);
  if (NULL == result) {
  ...
  long fs = (long) fread(result, 1, file_size, file);
  fclose(file);
  if (fs != file_size) {
  	printf("fread incorrect!\n");
  }
  ...
  if (memcmp("GMI GFF V1.0", result, 12) == 0) {
    ...
    // Handle GFF file
    if (file_size < 256) {
      printf("This file looks to small %ld\n", file_size);
    ...
    uint8_t* gff_data = (uint8_t *) malloc(gff_size);
    if (NULL == gff_data) {
      printf("Not enough RAM.\n");
      free(result);
      result = NULL;
